...
public class ProtocoleCercle implements Protocole {
    ...
    @Override
    public String toSVG(Cellule[][] qr, int taille, int tailleCarre, int dimP) {// taille est la taille des pixel des carre du svg
        int rayon = taille / 2; 
            ...
            for (int j = 0; j < tailleCarre; j++) {
                for (int m = 0; m < dimP; m++) { 
                    for (int n = 0; n < dimP; n++) { 
                        ...
                        svg.append("<circle cx=\"").append(x + rayon).append("\" cy=\"").append(y + rayon)
                        .append("\" r=\"").append(rayon - epaisseurContour).append("\" fill=\"").append(color)
